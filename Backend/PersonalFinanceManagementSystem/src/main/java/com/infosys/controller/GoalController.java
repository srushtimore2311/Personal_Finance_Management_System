    ...
    public ResponseEntity<String> addGoal(@RequestBody Goal goal) {
        try {
            ...
            if (response != null) {
                ...
                return new ResponseEntity<>("Failed to add goal. Please try again.", HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } catch (Exception e) {
            // Log the exception (optional, if logging is set up)
            // e.printStackTrace();
            return new ResponseEntity<>("An error occurred while adding the goal.", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<List<Goal>> getGoalByUsername(@RequestParam String username) {
        try {
            ...
            if (response != null && !response.isEmpty()) {
                ...
                return new ResponseEntity<>(response, HttpStatus.OK); // Return empty list with 200 OK
            }
        } catch (Exception e) {
            // Log the exception (optional, if logging is set up)
            // e.printStackTrace();
